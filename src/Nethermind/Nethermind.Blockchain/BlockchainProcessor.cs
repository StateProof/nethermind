    ...
    public class BlockchainProcessor : IBlockchainProcessor
    {
        ...
        public void Process(Block suggestedBlock)
        {
            ...
            if (chunkMicroseconds > 10 * 1000 * 1000 || _wasQueueEmptied) // 10s
            {
                ...
                if(_logger.IsInfoEnabled) _logger.Info($"Processed blocks up to {suggestedBlock.ToString(Block.Format.Short),9} in {(chunkMicroseconds == 0 ? -1 : chunkMicroseconds / 1000),7:N0}ms, tx={chunkTx,5} mgas={chunkMGas,8:F2}, mgasps={mgasPerSecond,7:F2}, txps={txps,7:F2}, total mgasps={totalMgasPerSecond,7:F2}, queue={_blockQueue.Count}");
